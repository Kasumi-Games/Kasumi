...
def render(data: list) -> Image.Image:
    '''
    渲染BanG Dream谱面
...
def non_slice_render(data: list) -> Image.Image:
    """不切片渲染"""
    chart = utils.preprocess_chart(data)
    height = utils.get_height(chart)
    chart_img, lane_range = utils.get_lanes(height, chart)
    ...
    bg = Image.new("RGBA", chart_img.size, BG_COLOR)

    return utils.paste(bg, chart_img, (0, 0))
