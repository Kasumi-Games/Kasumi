...
class Utils:
    def __init__(self) -> None:
        ...
        flick = Image.open(f"plugins/guess_chart/resources/{SKIN}/flick.png")
        left = Image.open(f"plugins/guess_chart/resources/{SKIN}/left.png")
        right = Image.open(f"plugins/guess_chart/resources/{SKIN}/right.png")
        left_arrow = Image.open(f"plugins/guess_chart/resources/{SKIN}/left_arrow.png")
        right_arrow = Image.open(
            f"plugins/guess_chart/resources/{SKIN}/right_arrow.png"
        )
        self.skin = {
            "flick": flick.convert("RGBA").resize(
                (
                    self.note_size[0],
                    int(flick.height / flick.width * self.note_size[0]),
                ),
                Image.Resampling.BICUBIC,
            ),
            "tap": Image.open(f"plugins/guess_chart/resources/{SKIN}/tap.png")
            .convert("RGBA")
            .resize(self.note_size, Image.Resampling.BICUBIC),
            "tap_white": Image.open(
                f"plugins/guess_chart/resources/{SKIN}/tap_white.png"
            )
            .convert("RGBA")
            .resize(self.note_size, Image.Resampling.BICUBIC),
            "tap_skill": Image.open(
                f"plugins/guess_chart/resources/{SKIN}/tap_skill.png"
            )
            .convert("RGBA")
            .resize(self.note_size, Image.Resampling.BICUBIC),
            "slide": Image.open(f"plugins/guess_chart/resources/{SKIN}/slide.png")
            .convert("RGBA")
            .resize(self.note_size, Image.Resampling.BICUBIC),
            "sep": Image.open(f"plugins/guess_chart/resources/{SKIN}/sep.png")
            .convert("RGBA")
            .resize(self.note_size, Image.Resampling.BICUBIC),
            "long": Image.open(f"plugins/guess_chart/resources/{SKIN}/long.png")
            .convert("RGBA")
            .resize((self.note_size[0], 1), Image.Resampling.BICUBIC),
            "left": left.convert("RGBA").resize(
                (self.note_size[0], int(left.height / left.width * self.note_size[0])),
                Image.Resampling.BICUBIC,
            ),
            "right": right.convert("RGBA").resize(
                (
                    self.note_size[0],
                    int(right.height / right.width * self.note_size[0]),
                ),
                Image.Resampling.BICUBIC,
            ),
            "left_arrow": left_arrow.convert("RGBA").resize(
                (
                    int(
                        left_arrow.width
                        / left_arrow.height
                        * (left.height / left.width * self.note_size[0])
                    ),
                    int(left.height / left.width * self.note_size[0]),
                ),
                Image.Resampling.BICUBIC,
            ),
            "right_arrow": right_arrow.convert("RGBA").resize(
                (
                    int(
                        right_arrow.width
                        / right_arrow.height
                        * (right.height / right.width * self.note_size[0])
                    ),
                    int(right.height / right.width * self.note_size[0]),
                ),
                Image.Resampling.BICUBIC,
    ...
    def paste(
        self, background: Image.Image, overlay: Image.Image, box: tuple
    ) -> Image.Image:
        """
        ...
        mask_draw.rectangle(
            (paste_x, paste_y, paste_x + overlay.width, paste_y + overlay.height),
            fill=255,
        )
    ...
    def _get_lighter_color(self, color: tuple) -> tuple:
        ...
        if len(color) == 3:
            return (
                r + self.color_light_value,
                g + self.color_light_value,
                b + self.color_light_value,
            )
        return (
            r + self.color_light_value,
            g + self.color_light_value,
    ...
    def get_lanes(self, height: int, chart: list):
        ...
        draw.rectangle(
            (self.frame_width // 2, 0, self.frame_width, height),
            fill=self._get_lighter_color(self.frame_color),
        )
        # 右frame
        draw.rectangle(
            (width - self.frame_width, 0, width, height), fill=self.frame_color
        )
        draw.rectangle(
            (width - self.frame_width, 0, width - self.frame_width // 2, height),
            fill=self._get_lighter_color(self.frame_color),
        )
        ...
        for idx, data in enumerate(chart):
            ...
            if (
                ...
                for idx_c, data_c in enumerate(data["connections"]):
                    chart[idx]["connections"][idx_c]["time"] = self.get_note_time(
                        data_c["beat"], timepoints
                    )
                    chart[idx]["connections"][idx_c]["pixel"] = int(
                        chart[idx]["connections"][idx_c]["time"] * self.pps
                    )
    ...
    def get_height(self, preprocessed_chart: list) -> int:
        ...
        data = next(
            (
                ...
                or any(c.get("beat") == max_beat for c in d.get("connections", []))
            ),
            None,
        )
        # 最后一个note
    ...
    def get_bpm_data(self, chart: dict):
        ...
        for data in chart:
            if data["type"] == "BPM":
                bpm_data.append(
                    {"bpm": data["bpm"], "pixel": data["pixel"], "beat": data["beat"]}
                )
        return bpm_data

    def draw_measure_lines(
        self, bpm_data: dict, draw: ImageDraw.ImageDraw, width: int, height: int
    ):
        """绘制小节线"""
        bpm_data = sorted(
            bpm_data, key=lambda x: x.get("beat", float("inf"))
        )  # 按beat排序
        for idx, data in enumerate(bpm_data):
            ...
            if int(length / pixel_per_beat) + 1 > 0:  # 属于该BPM范围的拍数 > 0
                for i in range(int(length / pixel_per_beat) + 1):
                    y1 = (
                        int(height - (y + self.bpm_line_width // 2))
                        if int(height - (y + self.bpm_line_width // 2)) != height
                        else height - self.bpm_line_width
                    )
                    y2 = (
                        int(height - (y - self.bpm_line_width // 2))
                        if int(height - (y - self.bpm_line_width // 2)) != height
                    ...
                    draw.rectangle(
                        (
                            self.x_sep + self.frame_width,
                            y1,
                            width - self.frame_width - self.x_sep,
                            y2,
                        ),
                        self.bpm_line_color if i != 0 else self.bpm_line_light_color,
                    )
                    y += pixel_per_beat
            else:  # 属于该BPM范围的拍数 < 0
    ...
    def draw_double_tap_lines(
        self, simplified_chart: dict, draw: ImageDraw.ImageDraw, height
    ):
        """绘制双押线"""
        for data_x in simplified_chart:
            for data_y in simplified_chart:
                if data_x["beat"] == data_y["beat"]:
                    x1, y1 = self.x_offset + int(
                        self.frame_width + (data_x["lane"] + 0.5) * self.lane_width
                    ), height - (data_x["pixel"] + self.double_beat_line_width // 2)
                    x2, y2 = self.x_offset + int(
                        self.frame_width + (data_y["lane"] + 0.5) * self.lane_width
                    ), height - (data_y["pixel"] - self.double_beat_line_width // 2)
                    if x1 < x2 and y1 < y2:
                        draw.rectangle(
                            (self.x_sep + x1, y1, self.x_sep + x2, y2),
                            self.double_beat_line_color,
                        )
    ...
    def draw_notes(self, chart: dict, chart_img: Image.Image, height: int):
        ...
        for data in chart:
            ...
            if data["type"] == "Single":
                ...
                if note_type == "tap":
                    if self.blue_white_tap and not (
                    ...
                    chart_img.paste(
                        ...
                        (
                            ...
                            + int(
                                ...
                                + (
                                    self.frame_width
                                    + (data["lane"] + 0.5) * self.lane_width
                                    - self.skin[note_type].width // 2
                                )
                                - self.sep_line_width
                            ),
                            height - (data["pixel"] + self.skin[note_type].height // 2),
                        ),
                        self.skin[note_type],
                    )
                else:
                    ...
                    chart_img.paste(
                        ...
                        (
                            ...
                            height
                            - (data["pixel"] + self.skin[note_type].height // 2)
                            - self.flick_offset,
                        ),
                        self.skin[note_type],
                    )
                        ...
                        + int(
                            ...
                            * (
                                (
                                    note_img.width
                                    if note_symbol == 1
                                    else self.skin[f"{note_type}_arrow"].width
                                )
                                + self.directional_arrow_offset
                            )
                        ),
                        y,
                    ),
                ...
                for idx_c, data_c in enumerate(data["connections"]):
                    ...
                    if idx_c != len(data["connections"]) - 1:
                        ...
                        x1 = self.x_offset + (
                            self.frame_width
                            + (data_c["lane"] + 0.5) * self.lane_width
                            - self.skin[note_type].width / 2
                        )
                        x2 = self.x_offset + (
                            self.frame_width
                        ...
                        for y in range(
                            ...
                            chart_img.paste(
                                self.skin[note_type],
                                (
                                    self.x_sep + int(x) - self.sep_line_width,
                                    height - int(y),
                                ),
                                self.skin[note_type],
                            )
    ...
    def draw_bpm_texts(
        ...
        for idx, data in enumerate(bpm_data):
            ...
            y = int(
                ...
                - data["pixel"]
                - (
                    self.font.getbbox("114514")[3]
                    if idx == 0
                    else self.font.getbbox("114514")[3] / 2
                )
            )
    ...
    def draw_time_texts(self, draw: ImageDraw.ImageDraw, height: int):
        ...
        for i in range(0, height, five_seconds_pixel):
            ...
            y = int(
                ...
                - i
                - (
                    self.font.getbbox(text)[3]
                    if i == 0
                    else self.font.getbbox(text)[3] / 2
                )
            )
            draw.text((x, y), text, self.time_text_color, self.font)

    def draw_note_num(
        self, draw: ImageDraw.ImageDraw, width: int, height: int, chart: list
    ):
        """绘制物量信息"""
    ...
    def process_image(self, original_image: Image.Image):
        ...
        combined_image = Image.new(
            "RGBA", ((width - self.x_sep) * num_slices, self.slice_height)
        )
